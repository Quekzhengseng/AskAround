...
import SurveyBuilder, { QUESTION_TYPES, QUESTION_TYPE_LABELS } from "./SurveyBuilder"; // Import constants
...
const SurveyCreatorPage = () => {
  ...
  const fetchAllSurveys = async () => {
    ...
    try {
      ...
      alert("Error fetching surveys. Please check the console or try again later.");
    }
  };
  ...
  const triggerSaveOrUpdate = async (isPublishing = false) => {
    ...
    try {
      ...
      if (surveyIdToPublish) { // UPDATE existing survey
          console.log(`Updating survey ${surveyIdToPublish}`, payload);
          response = await SurveyPublisherAPI.updateSurvey(surveyIdToPublish, payload);
          ...
          if (!surveyIdToPublish) throw new Error("Create failed: No survey ID returned from API.");
      ...
      console.error(`Error ${isPublishing ? "publishing" : "saving"} survey:`, err);
      alert(`Error: Could not ${isPublishing ? "publish" : "save"} survey. ${err.message}`);
    }
  };
  ...
  const handleDeleteSurvey = async (surveyId) => {
    if (!confirm("Are you sure you want to permanently delete this survey?")) return;
    try {
  ...
  const handleAddQuestion = useCallback((type) => {
      ...
      if (surveyBuilderRef.current) {
          ...
          console.error("Attempted to add question before SurveyBuilder ref was ready.");
      }
  }, []); // Dependency array is empty as ref doesn't change

  // --- Render Logic ---
  return (
    <div className="min-h-screen bg-[#f9fafb] text-[#111827] font-sans"> {/* bg-gray-50 text-gray-900 */}
      {/* --- Header --- */}
      <header className="sticky top-0 z-20 bg-white/90 backdrop-blur-sm border-b border-gray-200/80">
        <div className="container mx-auto px-4 h-16 flex justify-between items-center">
           ...
           <div className="flex-1 flex justify-start">
              {isCreating ? (
                 <button
                   onClick={() => { setIsCreating(false); setEditingSurvey(null); }}
                 ...
                 <span className="font-semibold text-lg text-gray-800">AskAround</span>
              )}
           </div>
           {/* Center Section (Title in List View) */}
           <div className="flex-1 text-center">
              {!isCreating && (
                  <h1 className="text-xl font-semibold text-[#111827]">Manage Surveys</h1>
              )}
           </div>
           {/* Right Section */}
              ...
              {isCreating ? (
                  ...
                  <div className="flex items-center gap-3">
                      ...
                      >
                         ...
                         {editingSurvey?.is_published ? 'Update & Publish' : 'Publish Survey'}
                      </button>
                  </div>
               ) : (
               ...
               <Link href="/profile" className="p-2 rounded-full text-gray-500 hover:bg-gray-100 hover:text-gray-800 transition-colors" aria-label="Profile">
                    <svg className="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>
               </Link>
           </div>
        </div>
      </header>
        ...
        {isCreating ? (
           ...
           <div className="flex flex-col lg:flex-row gap-8">
               ...
               <aside className="w-full lg:w-56 xl:w-64 flex-shrink-0">
                   <div className="sticky top-24 space-y-2"> {/* Adjust top offset based on header height */}
                       <h3 className="text-xs font-semibold uppercase text-[#9ca3af] mb-3 px-1">Add Question</h3> {/* text-subtle */}
           ...
           <>
             ...
             {allSurveys.length > 0 ? (
                 ...
                 {allSurveys.map((survey) => (
                   ...
                   >
                     ...
                     <div className="p-4 flex-grow">
                       <h3 className="font-medium text-[#111827] truncate mb-1 group-hover:text-[#7c3aed] text-sm" title={survey.title}> {/* text-main hover:purple */}
                         {survey.title || "Untitled Survey"}
                       </h3>
                       <p className="text-xs text-[#4b5563] mb-3 line-clamp-2 h-8"> {/* text-secondary */}
                         {survey.description || <span className="italic text-gray-400">No description</span>}
                       </p>
                       <div className="text-xs text-[#9ca3af] flex justify-between items-center"> {/* text-subtle */}
                         <span>{survey.questions?.length || 0} questions</span>
                         {/* Status Badge */}
                         {survey.hasOwnProperty('is_published') && (
                           <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-medium ${survey.is_published ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                     ...
                     <div className="p-2 bg-gray-50/30 border-t border-gray-100 flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                       <button onClick={() => handleEditSurvey(survey)} className="text-xs px-2 py-1 rounded text-blue-600 hover:bg-blue-100 transition-colors" title="Edit Survey">Edit</button>
                       <button onClick={() => handleDeleteSurvey(survey.id)} className="text-xs px-2 py-1 rounded text-red-500 hover:bg-red-100 transition-colors" title="Delete Survey">Delete</button>
                     </div>
                   </div>
                 ))}
               </div>
               ...
               <div className="text-center py-20">
                 ...
                 <h3 className="mt-2 text-lg font-medium text-[#111827]">No surveys yet</h3> {/* text-main */}
                 <p className="mt-1 text-sm text-[#4b5563]">Create your first survey to get started.</p> {/* text-secondary */}
                 <div className="mt-6">
                   <button onClick={handleCreateNewSurvey} className="bg-[#7c3aed] hover:bg-[#5b21b6] text-white py-1.5 px-4 rounded-md text-sm font-medium transition-colors"> Create Survey </button> {/* purple */}
                 </div>
               </div>
             )}
           </>
      ...
      {/* Footer */}
      <footer className="py-6 text-center text-[#9ca3af] text-xs"> {/* text-subtle */}
